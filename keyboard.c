...
#define USB_VENDOR	0x077a
#define USB_PRODUCT	0x100c
...
#define EP_NUM_INT_IN	0x01
#define EP_NUM_INT_OUT	0x02
...
char usb_hid_report[] = {
	...
	0x75, 0x08, 		//Report, Size, (8)
	0x95, 0x3f, 		//Report, Count, (63)
	0x82, 0x02, 		//Input, (Data,Var,Abs)
	0x01, 0xc0, 		//End, Collection
	0xc0			//End, Collection
};
...
void process_eps_info(int fd) {
	...
	int int_in_addr = usb_endpoint_num(&usb_endpoint_in);
	int int_out_addr = usb_endpoint_num(&usb_endpoint_out);
...
bool ep0_request(int fd, struct usb_raw_control_event *event,
	...
	case USB_TYPE_STANDARD:
		...
		case USB_REQ_GET_DESCRIPTOR:
			...
			break;
		case USB_REQ_SET_CONFIGURATION:
			ep_int_in = usb_raw_ep_enable(fd, &usb_endpoint_in);
			ep_int_out = usb_raw_ep_enable(fd, &usb_endpoint_out);
